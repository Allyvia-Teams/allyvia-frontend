...
Brand analysis now inspects logo lettering before general design-reference imagery and site CSS. The backend returns a structured typography report: suspected family, supported match, source, confidence and visible letterform reasoning. Image-based candidates are explicitly visual matches, never verified original fonts. A medium/high-confidence supported logo match takes priority over general website typography when tailoring. Low-confidence or unsupported suggestions retain the existing font (or use a supported CSS declaration); licensed custom fonts remain unchanged. The selected family applies to display headings and editable wordmarks and is stored with the brand kit. Interface typography remains separately editable for readability.

The report loads a type specimen for review before tailoring. Website-only declarations are labeled as CSS evidence, which does not prove logo provenance. Custom fonts outside the curated library require the existing licensed font URL flow. Visual matching uses the configured Google model, with the current 12-family library as its supported-match shortlist. Exact identification of arbitrary raster lettering is not guaranteed.

Validation: 1,524 frontend tests; browser report-to-heading application verified against a controlled analysis response. The isolated backend has no visual model key, so live image recognition accuracy was not measured. The local playground explicitly notes that visual font analysis requires the signed-in backend.

## Real integrated execution — September 10, 2026

Both feature branches were run together: frontend localhost:3017 and backend localhost:8017, with an isolated local review merchant. A real JWT-authenticated browser flow (no intercepted API responses) uploaded a synthetic serif M logo, supplied exact colors and aimeleondore.com, invoked the actual Google model using the machine's existing credential, tailored the theme, and saved through PUT /company/theme/. After deleting both global and company theme caches, GET /company/theme/ restored the actual Logo component, identified display font and button preset. The accepted visual match was DM Serif Display; the site reported Sohne and DIN W01 Regular separately. No analysis warnings or browser page errors remained. Independent ORM inspection confirmed the persisted palette, logo and settings.

The temporary browser harness mounted the actual Branding editor and Logo with normal axios calls. This proves that flow, not navigation through every OS screen or every possible logo's recognition accuracy. The harness was removed after verification. `live-integration-result.json` is the non-secret result. Credentials remain outside the repositories. The earlier no-key and controlled-response limitations above describe earlier runs, and are superseded for this local execution. Feature branches remain unmerged and undeployed.
