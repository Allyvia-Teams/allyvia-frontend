...
- `buildImmersiveColors` clones `generateBrandPalette(...)` output (never modifies that function — frozen-token tests) and re-points the **ColorProps tokens** (`paper`, `grey50`; dark: `darkPaper`, `darkBackground`, `darkLevel1/2`) *before* slot-mapping, because three consumers bypass `background.*`.
