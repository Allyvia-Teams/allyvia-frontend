...
This matrix is the per-metric verification-state inventory called for in ALL-54's original ticket text, tracked separately from the Critical/High/Medium/Low findings below. Severity describes *what's wrong*; Status here describes *whether a row has been verified against ground truth yet*. Every row starts at `?` — including rows that already correspond to a code-read finding (H1/H2/H3/H5) — because those only become confirmed ✗ once they reproduce against the seeded tenant from ALL-55, not from code-reading alone. `Observed`/`Expected` are filled in by ALL-55 (ground truth) and ALL-56/ALL-57 (actual observed value), so both are placeholders until that work happens. When a row is confirmed ✗, it will link the finding ID (e.g. H1) and its `Fix:` sub-issue under ALL-15.

| # | Metric | UI location | Component | Endpoint | Source fields | Expected formula | Observed | Expected | Status | Notes |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | Revenue | Financial Statements → P&L tiles | `FinancialStatements.tsx` | `/profit/profit_and_loss/` | `metrics.revenue_total()` inputs (QB invoice/sales revenue) | `revenue = metrics.revenue_total(company, start, end)` | ALL-56 pending | ALL-55 pending | ? | Formula read in `profit/services.py::_get_local_profit_and_loss`; no issue found in code |
| 2 | COGS | Financial Statements → P&L tiles | `FinancialStatements.tsx` | `/profit/profit_and_loss/` | `metrics.cogs_total()` inputs | `cogs = metrics.cogs_total(company, start, end)` | ALL-56 pending | ALL-55 pending | ? | No issue found in code |
| 3 | Gross Profit | Financial Statements → P&L tiles | `FinancialStatements.tsx` | `/profit/profit_and_loss/` | derived | `gross_profit = revenue - cogs` | ALL-56 pending | ALL-55 pending | ? | No issue found in code |
| 4 | Total Expenses | Financial Statements → P&L tiles | `FinancialStatements.tsx` | `/profit/profit_and_loss/` | `metrics.expenses_total()` inputs | `total_expenses = metrics.expenses_total(company, start, end)` | ALL-56 pending | ALL-55 pending | ? | No issue found in code |
| 5 | Operating Expenses | Financial Statements → P&L tiles | `FinancialStatements.tsx` | `/profit/profit_and_loss/` | derived | `operating_expenses = max(0, total_expenses - cogs)` | ALL-56 pending | ALL-55 pending | ? | Intentional floor at 0; documented edge case |
| 6 | Net Income | Financial Statements → P&L tiles | `FinancialStatements.tsx` | `/profit/profit_and_loss/` | derived | `net_income = revenue - total_expenses` | ALL-56 pending | ALL-55 pending | ? | No issue found in code |
| 7 | Net Operating Income | Financial Statements → P&L tiles | `FinancialStatements.tsx` | `/profit/profit_and_loss/` | derived | `net_operating_income = gross_profit - operating_expenses` | ALL-56 pending | ALL-55 pending | ? | Diverges from Net Income only when COGS > total expenses (floor case) — deliberate |
| 8 | Gross Margin % | Financial Statements → P&L tiles | `FinancialStatements.tsx` | `/profit/profit_and_loss/` | derived | `marginOf(gross_profit, revenue)` via `formatPercent` | ALL-56 pending | ALL-55 pending | ? | Em-dash on zero revenue, verified against `financeFormat.test.ts` |
| 9 | Net Margin % | Financial Statements → P&L tiles | `FinancialStatements.tsx` | `/profit/profit_and_loss/` | derived | `marginOf(net_income, revenue)` via `formatPercent` | ALL-56 pending | ALL-55 pending | ? | Same helper as #8 |
| 10 | Total Assets | Financial Statements → Balance Sheet | `FinancialStatements.tsx` | `/profit/balance-sheet/` | `QBAccount.current_balance`, account type | Sum of `current_balance` across asset-type accounts | ALL-56 pending | ALL-55 pending | ? | Deliberately a current-balance snapshot, not point-in-time — see `basis`/`is_point_in_time` fields |
| 11 | Total Liabilities | Financial Statements → Balance Sheet | `FinancialStatements.tsx` | `/profit/balance-sheet/` | `QBAccount.current_balance`, account type | Sum across liability-type accounts | ALL-56 pending | ALL-55 pending | ? | Same snapshot caveat as #10 |
| 12 | Total Equity | Financial Statements → Balance Sheet | `FinancialStatements.tsx` | `/profit/balance-sheet/` | `QBAccount.current_balance`, account type | Sum across equity-type accounts | ALL-56 pending | ALL-55 pending | ? | Same snapshot caveat as #10; equation check (Assets = Liabilities + Equity) also to be verified here |
| 13 | Balance Sheet title date | Financial Statements → Balance Sheet card | `FinancialStatements.tsx` (line 241) | `/profit/balance-sheet/` | `balanceSheet.effective_date` | Display `effective_date` from backend; loading-state fallback is local "today" | ALL-56 pending | ALL-55 pending | ? | See L1 — loading fallback uses UTC-shifted `toISOString()` date |
| 14 | Current Ratio | Financial Statements → ratio tiles (line 47) | `FinancialStatements.tsx` | `/profit/balance-sheet/` | `currentAssets`, `currentLiabilities` | Should route through `ratioOf(currentAssets, currentLiabilities)` | ALL-56 pending | ALL-55 pending | ? | See M2 — inline calc shows "0.00" instead of undefined when liabilities are 0 |
| 15 | Debt to Equity | Financial Statements → ratio tiles (line 48) | `FinancialStatements.tsx` | `/profit/balance-sheet/` | `totalLiabilities`, `totalEquity` | Should route through `ratioOf(totalLiabilities, totalEquity)` | ALL-56 pending | ALL-55 pending | ? | See M2 — same bug; negative equity also falls into "0.00" |
| 16 | Cash In (Operating) | Financial Statements → Cash Flow | `FinancialStatements.tsx` | `/profit/cash-flow/` | `QBPayment.amount`, `is_voided` | Sum of `QBPayment` where `is_voided=False`, in range | ALL-56 pending | ALL-55 pending | ? | Correctly filtered; no issue found |
| 17 | Cash Out (Operating) | Financial Statements → Cash Flow | `FinancialStatements.tsx` | `/profit/cash-flow/` | `QBPurchase.amount`, `QBBillPayment.total_amount`, `is_voided` | Sum of `QBPurchase` (`is_voided=False`) + `QBBillPayment` (should also be `is_voided=False`) | ALL-56 pending | ALL-55 pending | ? | See H1 — `QBBillPayment` query is missing the `is_voided=False` filter |
| 18 | Net Cash Flow | Financial Statements → Cash Flow | `FinancialStatements.tsx` | `/profit/cash-flow/` | derived | `cash_in - cash_out` | ALL-56 pending | ALL-55 pending | ? | Inherits H1's overstatement via #17 |
| 19 | P&L Statement table | Financial Statements → P&L table | `FinancialStatements.tsx` | `/profit/profit_and_loss/`, `/profit/cost_of_goods_and_services/`, `/profit/gross_profit/` | same as #1-7 | Full line-item breakdown of #1-7 | ALL-56 pending | ALL-55 pending | ? | No issue found beyond the tile-level rows above |
| 20 | Balance Sheet table | Financial Statements → Balance Sheet table | `FinancialStatements.tsx` | `/profit/balance-sheet/` | same as #10-13 | Line items + Assets = Liabilities + Equity check | ALL-56 pending | ALL-55 pending | ? | No issue found beyond #10-13 |
| 21 | Cash Flow Statement table | Financial Statements → Cash Flow table | `FinancialStatements.tsx` | `/profit/cash-flow/` | same as #16-18 | Operating Activities row breakdown | ALL-56 pending | ALL-55 pending | ? | See H1 — inherits #17's overstatement |
| 22 | Total Invoices | Invoices → KPI tiles | `Invoices.tsx` | `/invoice/stats/` | `QBInvoice` count | Count of `QBInvoice` where `is_voided=False`, in range | ALL-56 pending | ALL-55 pending | ? | `InvoiceStatsView.get_stats()` correctly excludes voided |
| 23 | Total Amount | Invoices → KPI tiles | `Invoices.tsx` | `/invoice/stats/` | `QBInvoice.amount` | Sum of `QBInvoice.amount` where `is_voided=False`, in range | ALL-56 pending | ALL-55 pending | ? | Correctly filtered |
| 24 | Paid | Invoices → KPI tiles | `Invoices.tsx` | `/invoice/stats/` | `QBInvoice.status` | Count/sum where `status=paid`, `is_voided=False` | ALL-56 pending | ALL-55 pending | ? | No issue found |
| 25 | Unpaid | Invoices → KPI tiles | `Invoices.tsx` | `/invoice/stats/` | `QBInvoice.status` | Count/sum where `status=unpaid`, `is_voided=False` | ALL-56 pending | ALL-55 pending | ? | No issue found |
| 26 | Overdue | Invoices → KPI tiles | `Invoices.tsx` | `/invoice/stats/` | `QBInvoice.status`, due date | Count/sum where `status=overdue`, `is_voided=False` | ALL-56 pending | ALL-55 pending | ? | No issue found |
| 27 | Invoice Table | Invoices → table | `InvoiceTable.tsx` | `/invoice/` | `QBInvoice.*`, `is_voided` | Should default-exclude voided invoices, matching the KPIs above it | ALL-56 pending | ALL-55 pending | ? | See H2 — `InvoiceListView` has no default `is_voided` exclusion; status chip never shows voided |
| 28 | Total Expenses | Expenses → KPI tiles | `ExpenseKPIs.tsx` | expense stats endpoint | `QBBill.amount`, `QBPurchase.amount`, `is_voided` | `total_bills + total_purchases` (`QBPurchase` filtered `is_voided=False`; `QBBill` has no such field) | ALL-56 pending | ALL-55 pending | ? | See H3 — table below only shows Bills, not Purchases |
| 29 | Expense Count | Expenses → KPI tiles | `ExpenseKPIs.tsx` | expense stats endpoint | `QBBill`, `QBPurchase` counts | `bill_count + purchase_count` | ALL-56 pending | ALL-55 pending | ? | See H3 |
| 30 | Average Expense | Expenses → KPI tiles | `ExpenseKPIs.tsx` | expense stats endpoint | derived | `total_expenses / expense_count` (guarded against 0) | ALL-56 pending | ALL-55 pending | ? | No issue found in the formula itself |
| 31 | Top Category | Expenses → KPI tiles | `ExpenseKPIs.tsx` | expense stats endpoint | Bills vs Purchases totals | Simple binary comparison of Bills total vs Purchases total | ALL-56 pending | ALL-55 pending | ? | UX-naming observation only — not real QB categories, working as coded |
| 32 | Expense Table | Expenses → table | `ExpenseTable.tsx` | `/expense/` (Bills) | `QBBill.*` only | Should reflect the same Bills+Purchases scope as the KPIs above it | ALL-56 pending | ALL-55 pending | ? | See H3 — `fetchPurchasesList` exists but is never dispatched anywhere in the UI |
| 33 | Total Payments | Payments → KPI tiles | `Payments.tsx` | `/payment/summary/` | `QBPayment.amount`, POS sales, `is_voided` | `sum(QBPayment, is_voided=False) + pos_sales_total` | ALL-56 pending | ALL-55 pending | ? | Correctly filtered; intentionally blends QB payments with completed POS sales |
| 34 | Payment Count | Payments → KPI tiles | `Payments.tsx` | `/payment/summary/` | `QBPayment` count, POS sales count | `qb_count + pos_count` | ALL-56 pending | ALL-55 pending | ? | Same blended-source note as #33 |
| 35 | Average Payment | Payments → KPI tiles | `Payments.tsx` | `/payment/summary/` | derived | `total_payments / payment_count` (guarded against 0) | ALL-56 pending | ALL-55 pending | ? | No issue found |
| 36 | Success Rate | Payments → KPI tiles | `Payments.tsx` | `/payment/summary/` | `QBPayment.unapplied_amount`, POS count | `(payments with unapplied_amount=0 + pos_count) / payment_count * 100` | ALL-56 pending | ALL-55 pending | ? | No issue found |
| 37 | Payment Table | Payments → table | `PaymentTable.tsx` | `/payment/` | `QBPayment.*`, `is_voided` | Should default-exclude voided payments, matching the KPIs above it | ALL-56 pending | ALL-55 pending | ? | See H5 — `PaymentListView` has no `is_voided` handling at all, not even an override param; no voided indicator in the table |
| 38 | Recent Sales | POS Sales → KPI tiles | `POSSales.tsx` | `/pos/recent-orders/` | `orders.length` (client-computed) | Count of the last 10 orders returned | ALL-56 pending | ALL-55 pending | ? | Intentionally fixed at last-10, not date-scoped — see backend test `test_recent_orders_returns_last_ten_scoped_to_company` |
| 39 | Total Revenue | POS Sales → KPI tiles | `POSSales.tsx` | `/pos/recent-orders/` | `order.total` (client-computed) | Sum of `order.total` across the returned orders | ALL-56 pending | ALL-55 pending | ? | Same fixed-window caveat as #38 |
| 40 | Cash Sales | POS Sales → KPI tiles | `POSSales.tsx` | `/pos/recent-orders/` | `order.paymentMethod` (client-computed) | Count where `paymentMethod === 'cash'` | ALL-56 pending | ALL-55 pending | ? | Same fixed-window caveat as #38 |
| 41 | Card Sales | POS Sales → KPI tiles | `POSSales.tsx` | `/pos/recent-orders/` | `order.paymentMethod` (client-computed) | Count where `paymentMethod === 'card'` | ALL-56 pending | ALL-55 pending | ? | Same fixed-window caveat as #38 |
| 42 | POS Sales table (Recent 10) | POS Sales → table | `POSSales.tsx` | `/pos/recent-orders/` | `Order.*` | `recent_orders(company, limit=10)`, no date-range param | ALL-56 pending | ALL-55 pending | ? | Intentional design, test-confirmed — not a bug |
