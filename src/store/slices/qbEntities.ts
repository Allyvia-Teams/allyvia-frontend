...
const initialState: QBEntitiesState = Object.keys(qbEntityConfigs).reduce((acc, key) => {
...
const qbEntitiesSlice = createSlice({
  ...
  reducers: {
    setItems: (state, action: PayloadAction<{ entity: string; items: any[] }>) => {
      state[action.payload.entity].items = action.payload.items;
    },
    setPagination: (state, action: PayloadAction<{ entity: string; pagination: PaginationInfo }>) => {
      state[action.payload.entity].pagination = action.payload.pagination;
    },
    setLoading: (state, action: PayloadAction<{ entity: string; loading: boolean }>) => {
      state[action.payload.entity].loading = action.payload.loading;
    },
    ...
    setFilters: (state, action: PayloadAction<{ entity: string; filters: Record<string, any> }>) => {
      state[action.payload.entity].filters = action.payload.filters;
    },
    ...
    updateItem: (state, action: PayloadAction<{ entity: string; id: string; updates: any }>) => {
      const entityState = state[action.payload.entity];
      const index = entityState.items.findIndex((item) => item.id === action.payload.id);
      if (index !== -1) {
    ...
    removeItem: (state, action: PayloadAction<{ entity: string; id: string }>) => {
      const entityState = state[action.payload.entity];
      entityState.items = entityState.items.filter((item) => item.id !== action.payload.id);
    ...
    Object.entries(fetchThunks).forEach(([entityType, thunk]) => {
      builder
        .addCase(thunk.pending, (state) => {
          state[entityType].loading = true;
          state[entityType].error = null;
        })
        .addCase(thunk.fulfilled, (state, action) => {
          const { data } = action.payload as { entityType: string; data: any };
          state[entityType].loading = false;
          state[entityType].items = data.items || [];
          state[entityType].pagination = data.pagination || null;
          state[entityType].error = null;
        })
        .addCase(thunk.rejected, (state, action) => {
          const payload = action.payload as { entityType: string; error: string };
          state[entityType].loading = false;
          state[entityType].error = payload.error;
        });
    });
    ...
    Object.entries(syncThunks).forEach(([entityType, thunk]) => {
      builder
        ...
        .addCase(thunk.fulfilled, (state, action) => {
          ...
          state[entityType].loading = false;
          state[entityType].lastSynced = new Date().toISOString();
          state[entityType].error = null;
        })
        .addCase(thunk.rejected, (state, action) => {
          const payload = action.payload as { entityType: string; error: string };
...
export const { setItems, setPagination, setLoading, setError, setFilters, clearEntity, updateItem, removeItem } = qbEntitiesSlice.actions;
