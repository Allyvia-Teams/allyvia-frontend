...
import {
  ...
  autoMapFields,
  buildMappedCsv,
  downloadDemoExpenseCsv,
  createEmptyExpenseFieldMap
...
export const ExpenseCSVImportModal: React.FC<Props> = ({ open, onClose, onSuccess }) => {
  const dispatch = useDispatch();
  const { currentRole } = useSelector((s) => s.auth);
  const { expenseUploadProgress, expenseUploadStatus, expenseUploadResult } = useSelector((s) => s.finance);

  const [activeStep, setActiveStep] = React.useState(0);

  React.useEffect(() => {
    if (activeStep === 2 && (expenseUploadStatus === 'success' || expenseUploadStatus === 'error')) {
      setActiveStep(3);
    }
  }, [activeStep, expenseUploadStatus]);

  React.useEffect(() => {
    if (expenseUploadStatus === 'success') {
      onSuccess?.();
    }
  }, [expenseUploadStatus, onSuccess]);

  const [file, setFile] = React.useState<File | null>(null);
  const [csvHeaders, setCsvHeaders] = React.useState<string[]>([]);
  const [csvRows, setCsvRows] = React.useState<Record<string, any>[]>([]);
  const [fieldMap, setFieldMap] = React.useState<Record<ExpenseField, string>>(createEmptyExpenseFieldMap());
  ...
  React.useEffect(() => {
    if (open) {
      dispatch(resetExpenseUpload());
    }
  }, [open, dispatch]);

  const handleDownloadTemplate = async () => {
    try {
      const result = await dispatch(downloadExpenseCsvTemplate() as any);
  ...
  const onDrop = React.useCallback((accepted: File[]) => {
    ...
    Papa.parse(f, {
      ...
      complete: (res) => {
        ...
        const mappedFields = new Set(Object.keys(autoMapping).filter((k) => autoMapping[k as ExpenseField]));
  ...
  const handleNext = async () => {
    ...
    if (activeStep === 2) {
      if (!currentRole?.id) {
        setMappingErrors(['Select a company from the header before importing expenses.']);
        return;
      }
      if (currentRole.role_type !== 'admin') {
        setMappingErrors(['Admin access is required to import expenses via CSV.']);
        return;
      }
      if (csvRows.length === 0) {
      ...
      if (file) {
        ...
        dispatch(uploadExpenseCsvFile(backendFile) as any);
      }
      return;
    }
  ...
  const handleClose = () => {
    ...
    setCsvHeaders([]);
    setCsvRows([]);
    setFieldMap(createEmptyExpenseFieldMap());
    setMappingErrors([]);
    ...
    >
      ...
      <DialogContent sx={{ display: 'flex', gap: 2, p: 0, overflow: 'hidden' }} dividers>
        ...
        <Box sx={{ flex: 1, p: 2, overflowY: 'auto', maxHeight: '70vh' }}>
          {expenseUploadStatus === 'uploading' && (
            <Box sx={{ mb: 2 }}>
              <LinearProgress
                variant="determinate"
                value={expenseUploadProgress}
                sx={{
                  height: 8,
                  borderRadius: 4,
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mt: 1, textAlign: 'center' }}>
                Uploading... {expenseUploadProgress}% complete
              </Typography>
            </Box>
          )}
          ...
          {activeStep === 0 && (
            <StepUploadSelect
              ...
              onDownloadDemo={downloadDemoExpenseCsv}
            />
          )}
          ...
          {activeStep === 1 && (
            <StepMapColumns
              ...
              optionalFields={EXPENSE_FIELDS.filter((f) => !REQUIRED_FIELDS.includes(f)) as string[]}
              csvHeaders={csvHeaders}
              fieldMap={fieldMap as any}
              autoMappedFields={autoMappedFields}
              mappingErrors={mappingErrors}
              setFieldMap={(updater: (prev: Record<ExpenseField, string>) => Record<ExpenseField, string>) =>
              ...
              {expenseUploadStatus === 'uploading' ? (
                <Box>
                  ...
                  <Box sx={{ mb: 3 }}>
                    ...
                    />
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 1, textAlign: 'center' }}>
                      {expenseUploadProgress}% complete - Processing {csvRows.length} rows...
                    </Typography>
                  </Box>
                </Box>
              ) : (
                <StepValidatePreview fields={[...EXPENSE_FIELDS]} csvRows={csvRows} fieldMap={fieldMap as any} />
              )}
            </Box>
          )}
          ...
          {activeStep === 3 && (
            <StepImportResult
              upload={{
                inProgress: expenseUploadStatus === 'uploading',
                progress: expenseUploadProgress,
                lastResult: expenseUploadResult || null
              }}
            />
          )}
        </Box>
      ...
      <DialogActions>
        ...
        <Button
          onClick={handleNext}
          variant="contained"
          disabled={(activeStep === 0 && !file) || (activeStep === 2 && expenseUploadStatus === 'uploading')}
        >
          {activeStep < 2 ? 'Next' : activeStep === 2 ? (expenseUploadStatus === 'uploading' ? 'Uploading...' : 'Import') : 'Close'}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
export default ExpenseCSVImportModal;
