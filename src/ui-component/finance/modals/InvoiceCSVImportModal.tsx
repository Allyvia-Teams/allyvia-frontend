...
import {
  ...
  autoMapFields,
  buildMappedCsv,
  downloadDemoInvoiceCsv,
  createEmptyInvoiceFieldMap
...
export const InvoiceCSVImportModal: React.FC<Props> = ({ open, onClose, onSuccess }) => {
  const dispatch = useDispatch();
  const { currentRole } = useSelector((s) => s.auth);
  const { invoiceUploadProgress, invoiceUploadStatus, invoiceUploadResult } = useSelector((s) => s.finance);

  const [activeStep, setActiveStep] = React.useState(0);

  React.useEffect(() => {
    if (activeStep === 2 && (invoiceUploadStatus === 'success' || invoiceUploadStatus === 'error')) {
      setActiveStep(3);
    }
  }, [activeStep, invoiceUploadStatus]);

  React.useEffect(() => {
    if (invoiceUploadStatus === 'success') {
      onSuccess?.();
    }
  }, [invoiceUploadStatus, onSuccess]);

  const [file, setFile] = React.useState<File | null>(null);
  const [csvHeaders, setCsvHeaders] = React.useState<string[]>([]);
  const [csvRows, setCsvRows] = React.useState<Record<string, any>[]>([]);
  const [fieldMap, setFieldMap] = React.useState<Record<InvoiceField, string>>(createEmptyInvoiceFieldMap());
  ...
  React.useEffect(() => {
    if (open) {
      dispatch(resetInvoiceUpload());
    }
  }, [open, dispatch]);

  const handleDownloadTemplate = async () => {
    try {
      const result = await dispatch(downloadInvoiceCsvTemplate() as any);
  ...
  const onDrop = React.useCallback((accepted: File[]) => {
    ...
    Papa.parse(f, {
      ...
      complete: (res) => {
        ...
        const mappedFields = new Set(Object.keys(autoMapping).filter((k) => autoMapping[k as InvoiceField]));
  ...
  const handleNext = async () => {
    ...
    if (activeStep === 2) {
      if (!currentRole?.id) {
        setMappingErrors(['Select a company from the header before importing invoices.']);
        return;
      }
      if (currentRole.role_type !== 'admin') {
        setMappingErrors(['Admin access is required to Import Invoices via CSV.']);
        return;
      }
      if (csvRows.length === 0) {
      ...
      if (file) {
        ...
        dispatch(uploadInvoiceCsvFile(backendFile) as any);
      }
      return;
    }
  ...
  const handleClose = () => {
    ...
    setCsvHeaders([]);
    setCsvRows([]);
    setFieldMap(createEmptyInvoiceFieldMap());
    setMappingErrors([]);
    ...
    >
      ...
      <DialogContent sx={{ display: 'flex', gap: 2, p: 0, overflow: 'hidden' }} dividers>
        ...
        <Box sx={{ flex: 1, p: 2, overflowY: 'auto', maxHeight: '70vh' }}>
          {invoiceUploadStatus === 'uploading' && (
            <Box sx={{ mb: 2 }}>
              <LinearProgress
                variant="determinate"
                value={invoiceUploadProgress}
                sx={{
                  height: 8,
                  borderRadius: 4,
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mt: 1, textAlign: 'center' }}>
                Uploading... {invoiceUploadProgress}% complete
              </Typography>
            </Box>
          )}
          ...
          {activeStep === 0 && (
            <StepUploadSelect
              ...
              onDownloadDemo={downloadDemoInvoiceCsv}
            />
          )}
          ...
          {activeStep === 1 && (
            <StepMapColumns
              ...
              optionalFields={INVOICE_FIELDS.filter((f) => !REQUIRED_FIELDS.includes(f)) as string[]}
              csvHeaders={csvHeaders}
              fieldMap={fieldMap as any}
              autoMappedFields={autoMappedFields}
              mappingErrors={mappingErrors}
              setFieldMap={(updater: (prev: Record<InvoiceField, string>) => Record<InvoiceField, string>) =>
              ...
              {invoiceUploadStatus === 'uploading' ? (
                <Box>
                  ...
                  <Box sx={{ mb: 3 }}>
                    ...
                    />
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 1, textAlign: 'center' }}>
                      {invoiceUploadProgress}% complete - Processing {csvRows.length} rows...
                    </Typography>
                  </Box>
                </Box>
              ) : (
                <StepValidatePreview fields={[...INVOICE_FIELDS]} csvRows={csvRows} fieldMap={fieldMap as any} />
              )}
            </Box>
          )}
          ...
          {activeStep === 3 && (
            <StepImportResult
              upload={{
                inProgress: invoiceUploadStatus === 'uploading',
                progress: invoiceUploadProgress,
                lastResult: invoiceUploadResult || null
              }}
            />
          )}
        </Box>
      ...
      <DialogActions>
        ...
        <Button
          onClick={handleNext}
          variant="contained"
          disabled={(activeStep === 0 && !file) || (activeStep === 2 && invoiceUploadStatus === 'uploading')}
        >
          {activeStep < 2 ? 'Next' : activeStep === 2 ? (invoiceUploadStatus === 'uploading' ? 'Uploading...' : 'Import') : 'Close'}
        </Button>
      </DialogActions>
    </Dialog>
  );
