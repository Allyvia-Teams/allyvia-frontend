...
const CrmLeadsKpisWidget: React.FC<AnalyticsWidgetProps> = ({ isLoading }) => {
  const { crmOverview, loading } = useSelector((state: RootState) => state.analytics);

  return <CRMAnalyticsKPIs kpis={crmOverview?.kpis} isLoading={loading || isLoading} section="leads" />;
};

export default CrmLeadsKpisWidget;
