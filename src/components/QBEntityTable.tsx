...
export const QBEntityTable: React.FC<QBEntityTableProps> = ({
  ...
  }, [companyId, entityType, localFilters, searchTerm, page, pageSize, dateRange, dispatch]);
      ...
      try {
        ...
        if (response.data?.suggestions) {
          return response.data.suggestions.map((s: any) => ({
            value: s[config.displayField] || s.name || s.value,
            label: s[config.displayField] || s.name || s.label,
            type: 'exact' as const,
            count: s.count
          }));
        }
  ...
  const getStatValue = (stat: any) => {
    ...
    if (statsData && config.statsEndpoint) {
      // Map stat label to stats data field for Bills
      ...
      const field = statMap[stat.label];
      if (field !== undefined) {
        const value = statsData[field];
        // Format currency values
        if (field === 'total_amount' || field === 'total_balance') {
  ...
  const formatCellValue = (column: any, value: any) => {
    ...
    if (column.id.includes('amount') || column.id === 'balance' || column.id.includes('price')) {
        ...
        <Box sx={{ position: 'relative' }}>
          <Grid container spacing={3} sx={{ mb: 3 }}>
            {config.statCards.map((stat, index) => {
              ...
              const isMonetary =
                stat.field?.includes('amount') ||
                stat.field?.includes('balance') ||
          ...
          {config.filters.balance && (
            <AllyviaFilterSelect
              ...
              options={[
                ...
                ...(config.filters.balance.options?.map((option) => ({
                  value: option,
                  label:
                    option === 'has_balance'
                      ? 'Has Balance'
