...
export function beginCapture(storage: Storage, company: string, kind: CaptureKind, payload: CapturePayload): CaptureRecord {
...
export function failCapture(storage: Storage, record: CaptureRecord, status?: number) {
  if (status === 400 || status === 422) storage.removeItem(key(record.company, record.kind));
}
export function recoverCapture(storage: Storage, company: string, kind: CaptureKind): CapturePayload | null {
  const raw = storage.getItem(key(company, kind));
  return raw ? (JSON.parse(raw) as CaptureRecord).payload : null;
}
export function completeCapture(storage: Storage, record: CaptureRecord) { storage.removeItem(key(record.company, record.kind)); }
